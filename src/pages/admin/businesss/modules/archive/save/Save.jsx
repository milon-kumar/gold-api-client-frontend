...
const Save = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            module_slug: MODULES?.ARCHIVES || 'archives',
        ...
        try {
            ...
            if (response?.success) {
                toast.success(response?.message || "Archive saved successfully");
                navigate('/admin/archives');
            } else {
                toast.error(response?.message || "Failed to save archive");
            }
        } catch (error) {
            console.error('Error saving archive:', error);
            toast.error('Error saving archive data');
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <PageHeader
                title={id && id !== 'new' ? 'Edit Archive' : 'Add New Archive'}
                subtitle={id && id !== 'new' ? `Update archive information` : `Add a new archive to the module`}
                showBackButton={true}
                onBackClick={() => navigate('/admin/archives')}
                primaryAction={{
                    onClick: handleSubmit,
                    disabled: saving,
                    icon: 'save',
                    title: id && id !== 'new' ? 'Update Archive' : 'Create Archive'
                }}
            />
