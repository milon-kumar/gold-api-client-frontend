...
const Save = () => {
  ...
  // Fetch existing president message
  const {
    data: itemGetQuery,
    isLoading: itemGetLoading,
    refetch: refetchItem
  } = useApiQuery({
    url: `/admin/business-modules/${MODULES?.PRESIDENT_MESSAGE}`,
    params: {
      module_slug: MODULES?.PRESIDENT_MESSAGE || 'president-message',
      limit: 1,
    }
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response?.success) {
        toast.success(response?.message || "President message saved successfully");
        await refetchItem();
        setIsEditing(true);
        if (response?.data?.id) {
        ...
        toast.error(response?.message || "Failed to save president message");
      }
    } catch (error) {
      console.error('Error saving president message:', error);
      toast.error('Error saving president message data');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <PageHeader
        title={isEditing ? 'Update President Message' : 'Create President Message'}
        subtitle={isEditing ? 'Update the president\'s message' : 'Add a new president message'}
        showBackButton={false}
        primaryAction={{
          onClick: handleSubmit,
          disabled: saving,
          icon: 'save',
          title: isEditing ? 'Update Message' : 'Create Message'
        }}
      />
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="shadow-sm">
              <CardHeader className="border-b bg-gray-50/50">
                ...
                <CardDescription>
                  Enter the president's message details
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={handleInputChange}
                    placeholder="e.g., Leading with Vision and Purpose"
                    className="mt-1.5"
                  />
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={handleInputChange}
                    placeholder="Enter a brief introduction to the president's message"
                    rows="3"
                    className="mt-1.5"
                  />
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={handleInputChange}
                    placeholder="Enter the full president's message here..."
                    rows="12"
                    className="mt-1.5 resize-y min-h-[300px]"
                  />
                  <p className="text-xs text-muted-foreground mt-1.5">
                    Write the complete message from the president. This will be displayed on the main page.
                  </p>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <Card className="shadow-sm">
              <CardHeader className="border-b bg-gray-50/50">
                ...
                <CardDescription>
                  Upload president's photo (Max {setting?.setting?.item?.image_size || 5}MB)
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {preview ? (
            ...
            <Card className="shadow-sm">
              ...
              <CardContent className="space-y-4">
                <div>
                  <Label htmlFor="status" className="text-sm font-semibold">Message Status</Label>
                  <Select
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Only active messages will be displayed on the website.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-sm border-purple-200 bg-purple-50/30">
              ...
              <CardContent className="space-y-2 text-sm">
                <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                  <li>Keep the message personal and inspirational</li>
                  <li>Highlight key achievements and future goals</li>
                  <li>Address the audience directly</li>
                  <li>Use a warm and welcoming tone</li>
                  <li>Include a call to action or closing statement</li>
                </ul>
              </CardContent>
            </Card>
          </div>
