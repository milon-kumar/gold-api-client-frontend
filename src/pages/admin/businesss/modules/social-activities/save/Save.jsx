...
const Save = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      module_slug: MODULES?.SOCIAL_ACTIVITIES || 'social-activities',
    ...
    try {
      ...
      if (response?.success) {
        toast.success(response?.message || "Social activity saved successfully");
        navigate('/admin/social-activities');
      } else {
        toast.error(response?.message || "Failed to save social activity");
      }
    } catch (error) {
      console.error('Error saving social activity:', error);
      toast.error('Error saving social activity data');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <PageHeader
        title={id && id !== 'new' ? 'Edit Social Activity' : 'Add New Social Activity'}
        subtitle={id && id !== 'new' ? `Update social activity information` : `Add a new social activity to the module`}
        showBackButton={true}
        onBackClick={() => navigate('/admin/social-activities')}
        primaryAction={{
          onClick: handleSubmit,
          disabled: saving,
          icon: 'save',
          title: id && id !== 'new' ? 'Update Activity' : 'Create Activity'
        }}
      />
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="shadow-sm">
              <CardHeader className="border-b bg-gray-50/50">
                ...
                <CardDescription>
                  Enter the social activity details
                </CardDescription>
              </CardHeader>
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Mark this activity as featured
                    </p>
                  </div>
                  <Switch
